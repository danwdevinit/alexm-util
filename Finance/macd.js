...
function parseCSV(e,r,b){
    ...
    if(now.hour()>=16 && (now.day()>0 && now.day()<6) && now.isAfter(latest,'day')){
        console.log("It's past close on a weekday, but Yahoo hasn't added the data for today yet...")
        console.log("Fetching today's close...")
        var summaryUrl = "http://finance.yahoo.com/q?s="+s;
        request(summaryUrl,parseSummary);
    }else{
        ...
        console.log(latest.format("MM/DD/YY")+","+raw[0].split(",")[4]+","+thisMacd+","+thisTrigger)
    };
};

function parseSummary(e,r,b){
    var $ = cheerio.load(b),
    currentClose = parseFloat($('span.time_rtq_ticker span').text());
    data.unshift(currentClose);
    var macdArr = macd(data),
    thisMacd = macdArr[0],
    thisTrigger = macdArr[1];
    console.log("The MACD is equal to "+thisMacd.toFixed(2)+" and the trigger line is equal to "+thisTrigger.toFixed(2)+".")
    ...
    console.log("Copy and paste the following line into your log-book:")
    console.log(now.format("MM/DD/YY")+","+currentClose+","+thisMacd+","+thisTrigger)
};