...
parser = OptionParser()
parser.add_option("-i", "--input", dest="input", default="S:/Projects/Programme resources/Data/Data sets/Domestic Government Expenditure/Government budgets/Uganda/Total PDFs/",
...
def main():
    ...
    #2015-16
    print("2015-16...")
    paths = glob.glob(options.input+"2015-16/pdf/*.pdf")
    datasets = []
    for path in paths:
    ...
    #2014-15
    print("2014-15...")
    paths = glob.glob(options.input+"2014-15/pdf/*.pdf")
    for path in paths:
        basename = os.path.basename(path)
        inputname, inputextension = os.path.splitext(basename)
        print("Reading "+basename+"...")
            ...
            for j in range(0,elLen):
                ...
                if el.tag == "text":
                    ...
                    if not isTable:
                        ...
                        if trytext(el) in revenueTypes:
                            revenueType = trytext(el)
                        elif abs(left-43)<3 or abs(left-86)<3 or abs(left-59)<3 or abs(left-119)<3 or abs(left-92)<3:
                            #Find row by going backwards and forwards...
                            row = []
                            elTop = int(el.attrib['top'])
                            obj = {}
                            ...
                            if row[0]['text'] not in wrongText and row[0]['text'][:4] not in wrongText and len(row)>=2:
                                ...
                                metaObj = {}
                                metaObj["Revenue Source"] = rowArr[0]
                                metaObj["AB1314"] = rowArr[1]
                                metaObj["Receipts1314"] = rowArr[2]
                                metaObj["PB1415"] = rowArr[3]
                                ...
                                obj['Vote']=vote
                                obj['Revenue Type']=revenueType
                                obj['Value']=metaObj["AB1314"]
                                obj['Revenue Source']=metaObj["Revenue Source"]
                                output.append(obj)
                                #2014/15 Proposed Budget
                                obj = {}
                                obj['year']="2014/15 Proposed Budget"
                                obj['Government']="District Government"
                                obj['District']=district
                                obj['Vote']=vote
                                obj['Revenue Type']=revenueType
                                obj['Value']=metaObj["PB1415"]
                                obj['Revenue Source']=metaObj["Revenue Source"]
                                output.append(obj)
        datasets+=output
    #2013-14
    print("2013-14...")
    paths = glob.glob(options.input+"2013-14/pdf/*.pdf")
    for path in paths:
        basename = os.path.basename(path)
        inputname, inputextension = os.path.splitext(basename)
        print("Reading "+basename+"...")
        ...
        for i in range(0,pageLen):
            isTable = False
            isTableFlags = ["A. Revenue Performance and Plans","B: Detailed Estimates of Revenue"]
            page = pages[i]
            elLen = len(page)
            for j in range(0,elLen):
                el = page[j]
                    ...
                    if not isTable:
                        ...
                        if trytext(el) in revenueTypes:
                            ...
                            if row[0]['text'] not in wrongText and row[0]['text'][:4] not in wrongText and len(row)>=2:
                                ...
                                metaObj = {}
                                metaObj["Revenue Source"] = rowArr[0]
                                metaObj["AB1213"] = rowArr[1]
                                metaObj["Receipts1213"] = rowArr[2]
                                metaObj["PB1314"] = rowArr[3]
                                ...
                                obj['Vote']=vote
                                obj['Revenue Type']=revenueType
                                obj['Value']=metaObj["AB1213"]
                                obj['Revenue Source']=metaObj["Revenue Source"]
                                output.append(obj)
                                #2013/14 Proposed Budget
                                obj = {}
                                obj['year']="2013/14 Proposed Budget"
                                obj['Government']="District Government"
                                obj['District']=district
                                obj['Vote']=vote
                                obj['Revenue Type']=revenueType
                                obj['Value']=metaObj["PB1314"]
                                obj['Revenue Source']=metaObj["Revenue Source"]
                                output.append(obj)
        datasets+=output
    if options.debug:
    ...
    with open(options.output+"Revenue-12-16.csv", 'wb') as output_file:
