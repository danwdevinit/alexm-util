...
parser = OptionParser()
parser.add_option("-i", "--input", dest="input", default="S:/Projects/Programme resources/Data/Data sets/Domestic Government Expenditure/Government budgets/Uganda/Missing PDFs/2013-14/",
                help="Input pdf name", metavar="FILE")
parser.add_option("-o", "--output", dest="output", default="S:/Projects/Programme resources/Data/Data sets/Domestic Government Expenditure/Government budgets/Uganda/Missing PDFs/Data/",
...
def main():
        ...
        for i in range(0,pageLen):
            ...
            for j in range(0,elLen):
                ...
                if el.tag == "text":
                    ...
                    if not isTable:
                        if trytext(el) in ["Local Government Budget Estimates","Local Government Budget Framework Paper","Local Government Workplan"]:
                        ...
                        if trytext(el) in AorBs:
                        ...
                        elif abs(left-73)<6 or abs(left-133)<6 or abs(left-116)<6 or abs(left-121)<6 or abs(left-181)<6:
                            ...
                            if len(row)>=2 and re.search("[a-zA-Z\/\\\(\)_]","".join([item['text'] for item in row[1:]]))==None:
                                ...
                                rights = [406,506,637]
                                altRights = [465,565,696]
                                altaltRights = [510,610,741]
                                for r in range(len(rights)):
                                    ...
                                    for element in row[1:]:
                                        if abs(element['right']-right)<18 or abs(element['right']-altRight)<18 or abs(element['right']-altaltRight)<18:
    ...
    with open(options.output+"Expenditure-13-14-missing.csv", 'wb') as output_file:
