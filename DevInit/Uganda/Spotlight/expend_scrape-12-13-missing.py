...
parser = OptionParser()
parser.add_option("-i", "--input", dest="input", default="S:/Projects/Programme resources/Data/Data sets/Domestic Government Expenditure/Government budgets/Uganda/Missing PDFs/2012-13/",
...
def main():
        ...
        for i in range(0,pageLen):
            ...
            for j in range(0,elLen):
                ...
                if el.tag == "text":
                    ...
                    if not isTable:
                        if trytext(el) in ["Local Government Budget Estimates","Local Government Workplan"]:
                        ...
                        elif abs(left-73)<6 or abs(left-133)<6 or abs(left-116)<6 or abs(left-121)<6:
                            ...
                            if len(row)>=2 and re.search("[a-zA-Z\/\\\(\)_]","".join([item['text'] for item in row[1:]]))==None:
                                ...
                                for r in range(len(rights)):
                                    ...
                                    for element in row[1:]:
                                        if abs(element['right']-right)<18 or abs(element['right']-altRight)<18:
    ...
    with open(options.output+"Expenditure-12-13-missing.csv", 'wb') as output_file:
