                ...
                if el.tag == "text":
                    ...
                    if not isTable:
                        if trytext(el)=="Local Government Budget Framework Paper":
                            ...
                            next1 = el.getnext()
                            next2 = el.getnext().getnext()
                            vote = "" if prev1 is None else trytext(prev1)
                            district = "" if prev2 is None else trytext(prev2)
                            workplan = ""
                            if next1 is not None:
                                if trytext(next1)!="Summary: Department Performance and Plans by Workplan":
                                    workplan = trytext(next1)
                                else:
                                    if next2 is not None:
                                        workplan = trytext(next2)
                            if vote[:6]=="Vote: " and workplan[:9]=="Workplan ":
                                isTable = True
                    else:
