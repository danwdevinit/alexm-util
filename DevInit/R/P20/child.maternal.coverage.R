...
for(i in 2:length(dirs)){
  ...
  if(hrwd %in% latest_surveys){
    ...
    for(i in 1:length(krKeep)){
      krVar <- krKeep[i]
      perc.name <- paste0(krVar,".percent")
      perc.val <- length(kr[[krVar]][which(!is.na(kr[[krVar]]))])/nrow(kr)
      percentList[[perc.name]] <- perc.val
    }
...
write.csv(metaData,"dhs_child_coverage.csv",row.names=FALSE,na="")
...
for(i in 2:length(dirs)){
  ...
  if(basename(dir) %in% latest_surveys){
    ...
    if(length(chKeep)>0){
      for(i in 1:length(chKeep)){
        chVar <- chKeep[i]
        perc.name <- paste0(chVar,".percent")
        perc.val <- length(ch[[chVar]][which(!is.na(ch[[chVar]]))])/nrow(ch)
        percentList[[perc.name]] <- perc.val
      }
    }
    if(length(wmKeep)>0){
      for(i in 1:length(wmKeep)){
        wmVar <- wmKeep[i]
        perc.name <- paste0(wmVar,".percent")
        perc.val <- length(wm[[wmVar]][which(!is.na(wm[[wmVar]]))])/nrow(wm)
        percentList[[perc.name]] <- perc.val
      } 
    }
    dataList[[dataIndex]] <- data.frame(percentList)
...
#       for(i in 1:length(chKeep)){
#         chVar <- chKeep[i]
#         chDesc <- subset(ch.labs,name==chVar)$description
#         descriptionList[[dlIndex]] <- list(
#           var = chVar
#           ,desc = chDesc
#           ,filename = basename(dir)
#           )
...
#       for(i in 1:length(wmKeep)){
#         wmVar <- wmKeep[i]
#         wmDesc <- subset(wm.labs,name==wmVar)$description
#         if(length(wmDesc)==0){wmDesc=""}
#         descriptionList[[dlIndex]] <- list(
#           var = wmVar
#           ,desc = wmDesc
#           ,filename = basename(dir)
...
#       } 
#     }
#     dataList[[dataIndex]] <- rbindlist(descriptionList)
#     dataIndex <- dataIndex + 1
  }
}
...
write.csv(mics.meta,"mics_child_coverage.csv",row.names=FALSE,na="")
