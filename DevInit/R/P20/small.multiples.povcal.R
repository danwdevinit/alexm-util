...
all.plot.alpha <- ggplot(all.data,aes(x=income)) + geom_density(aes(fill=1,colour=1),alpha=0.3) + xlim(-3,3) + facet_wrap(~iso3,ncol=5) + theme_bw() + theme(legend.position="none") + labs(x="Adj. income",y="Density")
...
all.plot <- ggplot(all.data,aes(x=income)) + geom_density(aes(fill=1,colour=1),alpha=0.3) + xlim(-3,3) + facet_wrap(~iso3,ncol=5) + theme_bw() + theme(legend.position="none") + labs(x="Adj. income",y="Density")
