...
#                  as.is=TRUE,
  ...
  ir <- read.csv(paste0(hrwd,"/wm.csv"),as.is=TRUE,na.strings="",check.names=FALSE)
  ...
  names(hr)[which(names(hr)=="hc10eb")] <- "car"
  ...
  #Rename fridge var
  names(hr)[which(names(hr)=="hc9f")] <- "fridge"
  ...
  #Rename phone var
  names(hr)[which(names(hr)=="hc9e")] <- "phone"
  ...
  #Rename tv var
  names(hr)[which(names(hr)=="hc9c")] <- "tv"
