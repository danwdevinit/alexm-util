...
ha <- subset(ha, !(DONOR %in% c("UNHRC","UNRWA","UNICEF","WFP")))
...
for(i in 1:length(CERFyears)){
  ...
  toCERFtmps <- page %>%
    html_nodes("table") 
  #First table
    toCERFtmp <- toCERFtmps[1] %>%
      html_table(fill=TRUE)
    toCERFtmp <- toCERFtmp[[1]]
  ...
  if(exists("toCERF")){toCERF <- rbind(toCERF,toCERFtmp)}else{toCERF <- toCERFtmp}
  #Third table
  toCERFtmp <- toCERFtmps[3] %>%
    html_table(fill=TRUE)
  toCERFtmp <- toCERFtmp[[1]]
  if(length(toCERFtmp)>2){
    toCERFtmp <- toCERFtmp[,c(2,3)]
    toCERFtmp$obsTime <- year
    toCERFtmp$RECIPIENT <- "Central Emergency Response Fund"
    names(toCERFtmp) <- c("DONOR","obsValue","obsTime","RECIPIENT")
...
deflator <- read.csv("OECD DAC Deflators 2013.csv",na.strings="",as.is=TRUE)
deflator <- subset(deflator,RECIPIENT=="TOTAL DAC ")
deflator <- deflator[,c("obsTime","deflator")]
deflator <- transform(deflator,deflator=deflator/100)
keep <- c(3,4,5,6)
...
cerffundingconstant <- transform(cerffundingconstant,obsValue=obsValue/deflator)
keep <- c("DONOR","RECIPIENT","obsTime","obsValue")
